...
function parseBody(request, contentType, sizeLimit, tempDirectory, cb) {
    ...
    request.on("data", function(chunk) {
        //console.log(`Recieved data chunk (${chunk.length} bytes)`)
...
function buildResult(parsedData, contentType, cb) {
    ...
    if(parsedData.fileWriter) {
        parsedData.fileWriter.on("close", () => cb(null, parsedData));
    } else {
        cb(null, parsedData);
    }
}
...
function parseMultipart(chunk, parsedData) {
    //console.log("Parsing multipart chunk");
...
function writeLastPiece(chunk, boundary, parsedData) {
    //console.log("Writing last piece of data");
...
function closeOutput(parsedData) {
    ...
    if(parsedData.fileWriter) {
        ...
        parsedData.fileWriter.on("finish", () => parsedData.fileWriter = undefined);
    }
...
function getDataBoundaries(chunk, boundary) {
    ...
    //console.log(`Got ${boundaries.length} boundaries from chunk`);
    return boundaries;
}

function parseFormData(chunk) {
    //console.log("Analizing multipart header");
    ...
    //console.log(`Form header: NAME=${formDataMatch[2]}; filename=${formDataMatch[3]}`);
    return {
        chunk: cutChunk,
